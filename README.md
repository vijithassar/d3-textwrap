...
6) Similarly, note that the bounds should be subject to the same transforms as the text node. This isn't technically prohibited, but would be a huge pain to position properly because the rect node and the text node are subject to different transforms.
